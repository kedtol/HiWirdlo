...
public class Map 
{
	...
	public static abstract class MapDrawing
	{
		...
		private void drawMap(Graphics g, Texture SpriteSheet, int cellX, int cellY, int cellZ, int cam_i, int cam_i1, int camX, int camY, int camX_rs, int camY_rs, Camera camera)
		{
			...
			drawItemTexture(g,screen_px+padding_camera_x*((int) item_width_c)+camera.drawPadding,screen_py+padding_camera_y *((int) item_width_c),map[cellX][cellY][cellZ].contain,SpriteSheet,1f);
		}
	...
	public void draw(Graphics g, int _mode, Texture _texture)
	{
		for (int i = 0; i < maxPlayers; i++)
		{
			...
			if (_mode == 0)
			{
				iPlayer.camera.drawMapAlpha(g,_texture,iPlayer.camera);
			}
			else
			{
				iPlayer.camera.drawMapOrder(g, _texture,iPlayer.camera);
			}
